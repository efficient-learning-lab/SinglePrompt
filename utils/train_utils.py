...
def select_optimizer(opt_name, lr, model):
    ...
    if opt_name == "adam":
        ...
        opt = optim.SGD(
            model.parameters(), lr=lr, momentum=0.9, nesterov=True, weight_decay=0.0005
        )
    else:
...
def select_model(model_name, dataset, num_classes=None, selection_size=None, pos_prompt_=5, prompt_length_=10, logit_type=None, kwargs=None):
...
#! cifar and imageNet --> ViT model 
    ...
    if model_name == "vit":
        ...
        pos_prompt_ = list(range(pos_prompt_))
        print()
        print('Prompt Injected Layers: ', pos_prompt_)
        print('Prompt Length: ', prompt_length_)
        print('Logit type: ', logit_type)
        print()
        model = create_model('singlePrompt_vit', pretrained=True, num_classes=num_classes,
                             drop_rate=0.0, drop_path_rate=0.0, drop_block_rate=None,
                             pos_prompt=pos_prompt_, prompt_length=prompt_length_, logit_type=logit_type)
